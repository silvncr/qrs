...
from qrs.ext import letter_scores, max_length_full, wordlist_full
...
def build_letter_scores(
	...
	return next(
		(
			val for key, val in letter_scores.items()
			if name == key
		), {},
	)
...
def main() -> None:
	...
	# get arguments
	from jarguments.create import JArgument
	from jarguments.parse import JParser
	cmd_args = JParser(
		*{
			JArgument(
				name=arg,
